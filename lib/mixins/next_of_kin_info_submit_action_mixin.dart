...
mixin NextOfKinInfoSubmitActionMixin {
  BuildContext get context;

  bool get mounted;

  Future<bool> handleSubmit(NextOfKinRequestData request) async {
    closeKeyboardAction(context);
    AppSnackBar.of(context).loading();
    try {
      final user = await Registry.di.repository.auth.saveNextOfKinDetails(request);
      if (!mounted) {
        return false;
      }
