...
mixin PersonalInfoSubmitActionMixin {
  BuildContext get context;

  bool get mounted;

  Future<bool> handleSubmit(PersonalInfoRequestData request) async {
    closeKeyboardAction(context);

    AppSnackBar.of(context).loading();
    try {
      final user = await Registry.di.repository.auth.savePersonalInfo(request);
      if (!mounted) {
        return false;
      }
