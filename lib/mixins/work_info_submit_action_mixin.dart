...
mixin WorkInfoSubmitActionMixin {
  BuildContext get context;

  bool get mounted;

  Future<bool> handleSubmit(WorkDetailsRequestData request) async {
    closeKeyboardAction(context);
    AppSnackBar.of(context).loading();
    try {
      final user = await Registry.di.repository.auth.saveWorkDetails(request);
      if (!mounted) {
        return false;
      }
